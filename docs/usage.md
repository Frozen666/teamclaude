...
A **warning banner** sits at the top of the page and is empty unless something is wrong. It reports a session that has had several client requests in a row come back with nothing usable — the case that reads as zero tokens exactly like an idle session, and is otherwise invisible — plus an account that needs a person (a broken token or a disabled entry) and an account that is billing real money. A spent quota bucket, a rate-limit back-off and an upstream refusal are **not** reported: those clear themselves, and a banner that is always on is one nobody reads. With `proxy.sessionDetail` off the banner still fires, but cannot name the session.
